...
function displayHealthBar(charac: Entity, color) {
  let str = "";
  let str2 = "";
  ...
  if (color == "red") {
    ...
    console.log(c.blue(str));
}

function startFight() {
  for (let etage = 0; etage <= 8 && Player.hp > 0; etage++) {
    ...
    for (j = 0; Player.hp > 0 && Enemy.hp > 0; j++) {
      ...
      displayHealthBar(Enemy, "red");
      console.log(`${Player.name} HP: ${Player.hp}/${Player.hp_max}.`);
      displayHealthBar(Player, "green");
  ...
  for (let etage = 1; etage == 1 && Player.hp > 0; etage++) {
    for (i = 0; Player.hp > 0 && Boss.hp > 0; i++) {
      ...
      if (Boss.hp <= 0) {
        ...
        console.log("Félicitation Links tu as vaincu Ganon et sauver Zelda , peut-être que pour une fois tu auras un baiser.");
